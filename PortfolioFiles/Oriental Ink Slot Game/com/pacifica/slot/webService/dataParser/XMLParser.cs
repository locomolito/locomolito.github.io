...
namespace com.pacifica.slot.webService.dataParser{
	...
	{	
		...
		public XMLNode Parse(string content)
		{
			...
			//string nodeContents = ""; //COMMENTED BEC ITS NEVER USED
			...
			for (int i = 0; i < content.Length; i++)
			{
				...
				if (inElement)
				{
					if (collectNodeName)
					{
						...
						else if(c==SLASH && cn == GT){	//ADDED TO FIX XML ISSUE: <examplenodename /> would work, but <examplenodename/> did not.
							collectNodeName=false;
							i--;
						}
